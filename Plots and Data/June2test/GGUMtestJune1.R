load('June2sims.RData')
library(ggumR)

chain1 <- ggumMCMC(responseMatrix, rep(4, 20), 40000)
cat('Chain 1 complete!\n')
chain2 <- ggumMCMC(responseMatrix, rep(4, 20), 40000)
cat('Chain 2 complete!\n')
chain3 <- ggumMCMC(responseMatrix, rep(4, 20), 40000)
cat('Chain 3 complete!\n')
save(chain1, chain2, chain3, file='June2chains.RData')

tau0s <- seq(from=1041, to=1120, by=4)
tau1s <- tau0s + 1
tau2s <- tau1s + 1
tau3s <- tau2s + 1
inds <- c(seq(from=1, to=1000, by=10), seq(from=1001, to=40000, by=25))

pdf('thetaTraceJune2.pdf', width=9, height=26)
...
pdf('alphaTraceJune2.pdf', width=9, height=26)
layout(matrix(1:20, ncol=2))
...
pdf('deltaTraceJune2.pdf', width=9, height=26)
layout(matrix(1:20, ncol=2))
...
pdf('tau1TraceJune2.pdf', width=9, height=26)
layout(matrix(1:20, ncol=2))
invisible(sapply(tau1s, function(x){
  plot(inds, chain1[inds, x], type='l', col=rgb(0, 0, 1, alpha=0.5),
       main=paste('Tau1', which(tau1s == x)),
       xlab='Iteration', ylab='Value', ylim=c(-3, 3))
  lines(inds, chain2[inds, x], col=rgb(1, 0, 0, alpha=0.5))
  lines(inds, chain3[inds, x], col=rgb(0, 1, 0, alpha=0.5))
...
pdf('tau2TraceJune2.pdf', width=9, height=26)
layout(matrix(1:20, ncol=2))
...
pdf('tau3TraceJune2.pdf', width=9, height=26)
layout(matrix(1:20, ncol=2))
