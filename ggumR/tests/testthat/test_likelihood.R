...
set.seed(123)
response_matrix <- cbind(sample(c(0:4), 1, replace = TRUE), 
                         sample(c(0:4), 1, replace = TRUE), 
                         sample(c(0:4), 1, replace = TRUE)) 
alphas <- runif(3, 0, 2)
deltas <- runif(3, 0, 1)
thetas <- rnorm(1)
taus <- list(c(0, runif(4, 0, 1)), c(0, runif(4, 0, 1)), c(0, runif(4, 0, 1))) # There in problem here.
